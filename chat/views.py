...
class ChatView(View):
    def get(self, request):
        all_rooms = set(ChatMessage.objects.all().values_list('chatRoom', flat=True)) 
        my_rooms = set(ChatMessage.objects.filter(user=request.user.username).values_list('chatRoom', flat=True))
